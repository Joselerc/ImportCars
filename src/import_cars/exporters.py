...
class ExcelExporter:
    ...
    def _listings_to_dataframe(
        self, listings: List[NormalizedListing], include_metadata: bool
    ) -> pd.DataFrame:
        ...
        for listing in listings:
            row = {
                ...
                "url": listing.url,
                "scraped_at": listing.scraped_at.replace(tzinfo=None)
                ...
                "original_price": listing.price_original.amount
                if listing.price_original
                else None,
                "original_currency": listing.price_original.currency_code
                if listing.price_original
                else None,
                ...
                "month": listing.first_registration.month
                if listing.first_registration
                else None,
                ...
                "consumption_combined_l_100km": listing.consumption_l_100km.combined
                if listing.consumption_l_100km
                else None,
                "consumption_urban_l_100km": listing.consumption_l_100km.urban
                if listing.consumption_l_100km
                else None,
                "consumption_highway_l_100km": listing.consumption_l_100km.highway
                if listing.consumption_l_100km
                else None,
                # Ubicación
                ...
                "seller_rating_count": listing.seller.rating_count
                if listing.seller
                else None,
                ...
                "seller_vat_number": listing.seller.vat_number
                if listing.seller
                else None,
                "seller_dealer_id": listing.seller.dealer_id
                if listing.seller
                else None,
                # Descripción
                ...
                row.update(
                    {
                        ...
                        "publish_date": listing.metadata.publish_date.replace(
                            tzinfo=None
                        )
                        if listing.metadata.publish_date
                        else None,
                        "update_date": listing.metadata.update_date.replace(tzinfo=None)
                        if listing.metadata.update_date
                        else None,
...
class CSVExporter:
    ...
    def export_listings(
        self, listings: List[NormalizedListing], filename: Optional[str] = None
    ) -> str:
