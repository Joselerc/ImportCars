...
def _co2_value(row: dict) -> int | None:
    return _to_int(row.get("co2_g_km")) or _to_int(row.get("co2_original_g_km")) or _to_int(row.get("co2_inferred_g_km"))
...
def _pick_tipo(row: dict) -> TipoCompra:
    ...
    if best_break_even is not None and available:
        return min(available, key=lambda tipo: abs((available[tipo] or 0) - best_break_even))
...
def _cost_breakdown(row: dict) -> dict | None:
    ...
    if price is None:
        return None
    tipo = _pick_tipo(row)
    breakdown = import_calculator.calcular_costes_importacion(price, tipo, _co2_value(row))
    es_market_avg = _to_float(row.get("es_market_avg"))
    if es_market_avg is not None:
        breakdown.update(import_calculator.calcular_beneficio_venta(breakdown, es_market_avg))
    return breakdown
...
def _normalize_row(row: dict[str, str]) -> dict:
    ...
    data["display_title"] = data.get("title") or " ".join(filter(None, [data.get("make"), data.get("model"), data.get("variant_key")]))
    data["image_url"] = data.get("image_url") or ""
    data["cost_breakdown"] = _cost_breakdown(data) if data.get("source") == "mobile_de" else None
    return data
...
def _parse_csv(filepath: Path) -> dict:
    ...
    opportunities = [row for row in mobile_rows if row.get("opportunity_score") is not None]
    ...
    summary = {
        ...
        "best_margin": max((row.get("potential_margin_avg") or float("-inf") for row in opportunities), default=None),
        "top_score": opportunities[0].get("opportunity_score") if opportunities else None,
        "positive_count": sum(1 for row in opportunities if (row.get("potential_margin_avg") or 0) > 0),
        "exact_count": sum(1 for row in opportunities if str(row.get("comparable_match_level") or "").lower() == "exact"),
        "near_count": sum(1 for row in opportunities if str(row.get("comparable_match_level") or "").lower() == "near"),
        "broad_count": sum(1 for row in opportunities if str(row.get("comparable_match_level") or "").lower() == "broad"),
...
def _build_command(request: CompareRequest, export_name: str) -> list[str]:
    ...
    de_payload = {
        ...
        "min_price": request.de_min_price if request.de_min_price is not None else request.min_price,
        "max_price": request.de_max_price if request.de_max_price is not None else request.max_price,
        "min_year": request.de_min_year if request.de_min_year is not None else request.min_year,
        "max_year": request.de_max_year if request.de_max_year is not None else request.max_year,
        "min_mileage": request.de_min_mileage if request.de_min_mileage is not None else request.min_mileage,
        "max_mileage": request.de_max_mileage if request.de_max_mileage is not None else request.max_mileage,
        "min_power": request.de_min_power if request.de_min_power is not None else request.min_power,
        "max_power": request.de_max_power if request.de_max_power is not None else request.max_power,
    ...
    es_payload = {
        ...
        "min_price": request.es_min_price if request.es_min_price is not None else request.min_price,
        "max_price": request.es_max_price if request.es_max_price is not None else request.max_price,
        "min_year": request.es_min_year if request.es_min_year is not None else request.min_year,
        "max_year": request.es_max_year if request.es_max_year is not None else request.max_year,
        "min_mileage": request.es_min_mileage if request.es_min_mileage is not None else request.min_mileage,
        "max_mileage": request.es_max_mileage if request.es_max_mileage is not None else request.max_mileage,
        "min_power": request.es_min_power if request.es_min_power is not None else request.min_power,
        "max_power": request.es_max_power if request.es_max_power is not None else request.max_power,
...
def _report_entries(limit: int = 30) -> list[dict]:
    entries = []
    for path in sorted(_web_exports_dir().iterdir(), key=lambda item: item.stat().st_mtime, reverse=True):
        if not path.is_file():
            continue
        entries.append(
            {
...
def _page_context(request: Request, active_page: str, page_title: str, page_eyebrow: str) -> dict:
    return {
        "request": request,
...
async def dashboard(request: Request):
    return templates.TemplateResponse(
        request,
        "dashboard.html",
        _page_context(request, "dashboard", "Market Comparison Dashboard", "Germany [DE] -> Spain [ES] arbitrage analysis"),
    )
...
async def comparisons(request: Request):
    return templates.TemplateResponse(
        request,
        "comparisons.html",
        _page_context(request, "comparisons", "Comparison Studio", "Cross-market comparison workspace"),
    )
...
async def reports(request: Request):
    ...
    context = _page_context(request, "reports", "Saved Reports", "Recent exports and downloadable analysis files")
    context.update(
        {
            "report_items": items,
...
async def calculator(request: Request):
    context = _page_context(request, "calculator", "Import Calculator", "Germany to Spain cost simulator")
    context["calculator_defaults"] = {"purchase_price": 29500, "sale_price": 43990, "co2": 179, "seller_type": "EmpresaIVA"}
...
async def compare(request: CompareRequest) -> dict:
    ...
    if process.returncode != 0:
        raise HTTPException(status_code=500, detail=stderr.decode("utf-8", errors="replace") or "Error ejecutando comparar")
...
async def import_calculator_api(request: CalculatorRequest) -> dict:
    ...
    selected = import_calculator.calcular_costes_importacion(request.purchase_price, selected_tipo, request.co2)
    selected_profit = (
    ...
    for tipo in TipoCompra:
        costs = import_calculator.calcular_costes_importacion(request.purchase_price, tipo, request.co2)
        result = {"tipo": tipo.value, **costs}
        if request.sale_price is not None:
            result.update(import_calculator.calcular_beneficio_venta(costs, request.sale_price))
        scenarios.append(result)

    best = min(scenarios, key=lambda item: item["break_even"]) if scenarios else None
    return {
        "selected": {**selected, **(selected_profit or {}), "tipo": selected_tipo.value},
        "scenarios": scenarios,
        "best": best,
