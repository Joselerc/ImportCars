...
class MobileDeScraper(BaseScraper):
    ...
    async def search(
        self, *, query: dict[str, Any] | UnifiedFilters, limit: int | None = None
    ) -> SearchResult:
        ...
        async with async_playwright() as p:
            ...
            if not html:
                ...
                await browser.close()
                return SearchResult(
                    listings=[], total_listings=0, result_page=page, has_next=False
                )
        ...
        *,
        ...
        try:
            ...
            try:
                ...
                logger.debug("No apareció el banner de cookies de mobile.de")
            ...
            print(
                "DEBUG: Haciendo scroll gradual para activar lazy loading y capturar IDs..."
            )
            ...
            while scroll_position < total_height:
                ...
                await page.wait_for_timeout(
                    800
                )  # Esperar a que se disparen los requests
            ...
            print(
                f"DEBUG: Scroll completado. IDs interceptados: {len(self.intercepted_vehicle_ids)}"
            )
            ...
            logger.exception("Error durante la navegación con Playwright")
            await page.close()
            return None, None
    ...
    async def _extract_listings_from_html(
        ...
        *,
        ...
        print(
            f"DEBUG: Encontrados {len(listing_nodes)} anuncios en la página de resultados."
        )
        ...
        for i, node in enumerate(selected_nodes):
            ...
            ad_id = (
                id_match.group(1)
                if id_match
                else testid.replace("-link", "")
                if testid
                else f"mobile-de-{i + 1}"
            )

            title = (
                node.css_first("h2.ListingTitle_title__p3CnA").text(strip=True)
                if node.css_first("h2.ListingTitle_title__p3CnA")
                else None
            )
            # Limpiar el título de prefijos como "Patrocinado"
            if title:
                title = re.sub(
                    r"^(Patrocinado|Sponsored)\s*", "", title, flags=re.IGNORECASE
                ).strip()
            ...
            if title:
                ...
                clean_title = re.sub(
                    r"^(Nuevo|New)\s*", "", title, flags=re.IGNORECASE
                ).strip()
            ...
            price_text = (
                node.css_first("span.PriceLabel_mainPrice__3SZut").text(strip=True)
                if node.css_first("span.PriceLabel_mainPrice__3SZut")
                else None
            )
            price_bruto = None
            price_neto = None
            ...
            details_node = node.css_first(
                "div[data-testid='listing-details-attributes']"
            )
            details_text = (
                details_node.text(separator=" ", strip=True) if details_node else ""
            )

            mileage_match = re.search(r"([0-9\.,]+)\s*km", details_text, re.IGNORECASE)
            mileage = (
                int(mileage_match.group(1).replace(".", "").replace(",", ""))
                if mileage_match
                else None
            )
            ...
            if price_bruto and not price_neto:
                vat_rate = (
                    1.19 if location and location.get("country_code") == "DE" else 1.21
                )
        ...
        if not items:
            logger.warning(
                "No se encontraron anuncios en la página de resultados de mobile.de"
            )
    ...
    async def _get_real_urls_with_javascript(self, context) -> list[str]:
        ...
        try:
            ...
            print(
                f"DEBUG: JavaScript encontró {len(valid_urls)} URLs, {sum(1 for u in valid_urls if u)} válidas"
            )

            return valid_urls

        except Exception as exc:  # noqa: BLE001 - JavaScript del navegador externo
            logger.debug("Error obteniendo URLs con JavaScript: %s", exc)
            return []
    ...
    async def _get_real_urls_from_page(self, context) -> list[str]:
        ...
        try:
            ...
            print(
                f"DEBUG: Obtenidas {len(valid_urls)} URLs reales de {len(urls)} anuncios"
            )

            return valid_urls

        except Exception as exc:  # noqa: BLE001 - JavaScript del navegador externo
            logger.debug("Error obteniendo URLs reales: %s", exc)
            return []
    ...
    async def _scrape_detail_page(self, context, url: str) -> dict[str, Any]:
        ...
        try:
            ...
            for dt in dt_elements:
                ...
                dd = dt.css_first(
                    "+ dd"
                )  # El elemento dd que sigue inmediatamente al dt
                ...
                if testid and dd:
                    ...
                    if field_name:
                        ...
                        if field_name == "co2_emissions_g_km":
                        ...
                        elif field_name == "doors" or field_name == "previous_owners":
                            match = re.search(r"(\d+)", value_text)
                            details[field_name] = int(match.group(1)) if match else None
                            ...
                            if match:
                                ...
                                details[field_name] = (
                                    value_text  # Guardar texto si no se puede parsear
                                )
                        else:
        ...
        except Exception as exc:  # noqa: BLE001 - HTML externo no confiable
            logger.warning("Error procesando página de detalle %s: %s", url, exc)
        finally:
            await page.close()
    ...
    def _parse_response(
        self, data: dict[str, Any], *, page_number: int, page_size: int
    ) -> SearchResult:
    ...
    def _to_listing(self, node: dict[str, Any]) -> NormalizedListing | None:
        reg_info = node.get("firstRegistration")
        registration = (
            Registration(year=reg_info["year"], month=reg_info["month"])
            if reg_info
            else None
        )
        ...
        return NormalizedListing(
            ...
            price_original=Price(amount=node.get("price_eur"), currency_code="EUR")
            if node.get("price_eur")
            else None,
